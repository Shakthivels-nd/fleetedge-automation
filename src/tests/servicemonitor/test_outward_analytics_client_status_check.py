"""
Feature: Service Monitor — outwardAnalyticsClient Status Check
Description:
  Validate that service_mon correctly logs outwardAnalyticsClient's start, crash (SIGABRT),
  and stop (SIGTERM) events.

  Log tag ("outwardAnalyticsClientNRT") is CONFIRMED from real FE service_mon
  logs (grepped directly from the pod on 2026-09-15: "Service started:
  outwardAnalyticsClientNRT :"), unlike most of the other servicemonitor
  tests in this folder whose tags are still unverified guesses. Note the
  "NRT" suffix — service_mon logs this service under a different name than
  the supervisorctl service name "outwardAnalyticsClient"; see
  fleetedge_automation_exploration.md (SERVICEMONITOR TESTS) for the full
  tag-verification status across all 11.
"""

import time

LOG_DIR = "/home/ubuntu/.nddevice/log/service_mon"
SERVICE_NAME = "outwardAnalyticsClient"
LOG_TAG = "outwardAnalyticsClient"


def test_step1_verify_services_active(device):
    """STEP 1 — Verify service_mon and outwardAnalyticsClient are both RUNNING."""
...
def test_step2_restart_service(device):
    """STEP 2 — Restart outwardAnalyticsClient to trigger a start event."""
...
def test_step3_verify_service_started_log(device):
    """STEP 3 — Verify 'Service started: outwardAnalyticsClient' in service_mon logs."""
    start_ts = device.variables.get("restart_start_ts")
    found = device.search_log(LOG_DIR, f"Service started: {LOG_TAG}", start_ts, timeout=120, interval=5)
...
def test_step4_get_pid_and_send_sigabrt(device):
    """STEP 4 — Get PID of outwardAnalyticsClient and send SIGABRT (kill -6)."""
...
def test_step5_verify_service_error_log(device):
    """STEP 5 — Verify 'Service error: outwardAnalyticsClient' in service_mon logs (crash detected)."""
...
def test_step6_get_pid_and_send_sigterm(device):
    """STEP 6 — Get new PID of outwardAnalyticsClient (after crash recovery) and send SIGTERM."""
    pid = device.run(f"pidof {SERVICE_NAME} | tr ' ' '\\n' | sort -n | head -n 1")
...
def test_step7_verify_service_stopped_log(device):
    """STEP 7 — Verify 'Service stopped: outwardAnalyticsClient' in service_mon logs (graceful stop)."""
...
def test_step8_restart_and_verify_active(device):
    """STEP 8 — Restart outwardAnalyticsClient and verify both services are RUNNING again."""
    result = device.restart_service(SERVICE_NAME)
    assert result["status"] == "Pass", f"Failed to restart {SERVICE_NAME}: {result['output']}"
    time.sleep(10)
    result = device.is_service_active("service_mon")

"""
Feature: Service Monitor — unifiedAnalyticsClient Status Check
Description:
  Validate that service_mon correctly logs unifiedAnalyticsClient's start, crash (SIGABRT),
  and stop (SIGTERM) events.

  Log tag ("unifiedAnalyticsClient") is set to the service name itself — the user
...
LOG_TAG = "unifiedAnalyticsClient"


def test_step1_verify_services_active(device):
    """STEP 1 — Verify service_mon and unifiedAnalyticsClient are both RUNNING."""
...
def test_step2_restart_service(device):
    """STEP 2 — Restart unifiedAnalyticsClient to trigger a start event."""
...
def test_step3_verify_service_started_log(device):
    """STEP 3 — Verify 'Service started: unifiedAnalyticsClient' in service_mon logs."""
...
def test_step4_get_pid_and_send_sigabrt(device):
    """STEP 4 — Get PID of unifiedAnalyticsClient and send SIGABRT (kill -6)."""
...
def test_step5_verify_service_error_log(device):
    """STEP 5 — Verify 'Service error: unifiedAnalyticsClient' in service_mon logs (crash detected)."""
...
def test_step6_get_pid_and_send_sigterm(device):
    """STEP 6 — Get new PID of unifiedAnalyticsClient (after crash recovery) and send SIGTERM."""
    pid = device.run(f"pidof {SERVICE_NAME} | tr ' ' '\\n' | sort -n | head -n 1")
...
def test_step7_verify_service_stopped_log(device):
    """STEP 7 — Verify 'Service stopped: unifiedAnalyticsClient' in service_mon logs (graceful stop)."""
...
def test_step8_restart_and_verify_active(device):
    """STEP 8 — Restart unifiedAnalyticsClient and verify both services are RUNNING again."""
    result = device.restart_service(SERVICE_NAME)
    assert result["status"] == "Pass", f"Failed to restart {SERVICE_NAME}: {result['output']}"
    time.sleep(10)
    result = device.is_service_active("service_mon")

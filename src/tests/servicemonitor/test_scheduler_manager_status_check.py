"""
Feature: Service Monitor — scheduler_manager Status Check
Description:
  Validate that service_mon correctly logs scheduler_manager's start, crash (SIGABRT),
  and stop (SIGTERM) events.

  Log tag ("Scheduler") is CONFIRMED from real FE service_mon logs (grepped
  directly from the pod on 2026-09-15: "Service started: Scheduler"), unlike
  most of the other servicemonitor tests in this folder whose tags are still
  unverified guesses. This is the supervisor service "scheduler_manager"'s
  service_mon-visible name; see fleetedge_automation_exploration.md
  (SERVICEMONITOR TESTS) for the full tag-verification status across all 11.
"""

import time

LOG_DIR = "/home/ubuntu/.nddevice/log/service_mon"
SERVICE_NAME = "scheduler_manager"
LOG_TAG = "Scheduler"


def test_step1_verify_services_active(device):
    """STEP 1 — Verify service_mon and scheduler_manager are both RUNNING."""
...
def test_step2_restart_service(device):
    """STEP 2 — Restart scheduler_manager to trigger a start event."""
...
def test_step3_verify_service_started_log(device):
    """STEP 3 — Verify 'Service started: Scheduler' in service_mon logs."""
...
def test_step4_get_pid_and_send_sigabrt(device):
    """STEP 4 — Get PID of scheduler_manager and send SIGABRT (kill -6)."""
...
def test_step5_verify_service_error_log(device):
    """STEP 5 — Verify 'Service error: Scheduler' in service_mon logs (crash detected)."""
...
def test_step6_get_pid_and_send_sigterm(device):
    """STEP 6 — Get new PID of scheduler_manager (after crash recovery) and send SIGTERM."""
    pid = device.run(f"pidof {SERVICE_NAME} | tr ' ' '\\n' | sort -n | head -n 1")
...
def test_step7_verify_service_stopped_log(device):
    """STEP 7 — Verify 'Service stopped: Scheduler' in service_mon logs (graceful stop)."""
...
def test_step8_restart_and_verify_active(device):
    """STEP 8 — Restart scheduler_manager and verify both services are RUNNING again."""
    result = device.restart_service(SERVICE_NAME)
    assert result["status"] == "Pass", f"Failed to restart {SERVICE_NAME}: {result['output']}"
    time.sleep(10)
    result = device.is_service_active("service_mon")
